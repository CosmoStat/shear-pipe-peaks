...
EXAMPLE:
python test_gpr_full.py '/home/tersenov/shear-pipe-peaks' --nproc 5  
"""
)
...
for index_cosmo in range(num_cosmologies):
    ...
    # Plot the difference between the predicted and true peak counts
    plt.figure()
    plt.plot(test[0] - Peaks_Maps_mean[index_cosmo])
    plt.show()
    # save the plot
    plt.savefig(maindir+'/plot/gpr_test/test_gpr_{}.pdf'.format(index_cosmo))

# save the mean MAPE values
np.savetxt(maindir+'/output/gpr_test/mean_mape.txt', mape_values)