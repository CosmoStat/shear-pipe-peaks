...
# Function to compute multi-scale peak counts for a single tile
def compute_ms_peak_counts(tile_file):
...
for realization_files in collections_of_files:  
    ...
    for tile_file in realization_files:
        ...
        with Pool(processes=num_tiles_per_realization) as pool:
            # Compute multi-scale peak counts in parallel for each tile in this realization
            ms_peak_counts_realization = pool.map(compute_ms_peak_counts, realization_files)


        # Compute the average multi-scale peak counts vector for this realization
        average_ms_peak_counts = np.mean(ms_peak_counts_realization, axis=0)

        # Append the average multi-scale peak counts vector for this realization to the list
        MS_PC_data_vectors.append(average_ms_peak_counts)
